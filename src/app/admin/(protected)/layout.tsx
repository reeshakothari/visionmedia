...
import { LayoutDashboard, FileEdit, Inbox, ExternalLink, LogOut } from "lucide-react";
    ...
    <div className="flex min-h-screen bg-cream-alt">
      <aside className="fixed inset-y-0 left-0 z-40 hidden w-60 flex-col border-r border-navy/10 bg-navy text-white md:flex">
        ...
        <div className="space-y-1 border-t border-white/10 p-3">
          ...
          <form action={logoutAction}>
            <button
              type="submit"
              className="flex w-full items-center gap-2 rounded-lg px-3 py-2.5 text-xs font-medium text-white/60 transition-colors hover:bg-white/10 hover:text-white"
            >
      ...
      <div className="flex min-h-screen w-full flex-col md:pl-60">
        <header className="flex items-center justify-between border-b border-navy/10 bg-white px-4 py-3 md:hidden">
          ...
          <form action={logoutAction}>
            <button type="submit" className="text-xs font-semibold text-navy/60">
              Log out
            </button>
          </form>
        </header>
