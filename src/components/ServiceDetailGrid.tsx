        ...
        <Reveal>
          <div className="mx-auto mb-12 max-w-2xl text-center md:mb-16">
            ...
            <h2 className="font-display text-3xl text-navy sm:text-4xl md:text-[2.75rem]">
              <Field path="servicesHeading" value={heading} />
            </h2>
            <p className="mt-4 text-sm leading-relaxed text-muted-light sm:text-base">
              <Field path="servicesSubheading" value={subheading} />
            </p>
          </div>
        </Reveal>
            ...
            <Reveal key={i} delay={(i % 3) * 90}>
              <div className="hairline group h-full rounded-2xl bg-white p-7 transition-all duration-500 ease-out hover:-translate-y-1.5 hover:shadow-premium">
                ...
                <h3 className="mt-4 font-display text-lg text-navy">
                  <Field path={`services.${i}.title`} value={service.title} />
                </h3>
                <p className="mt-2 text-sm leading-relaxed text-muted">
                  <Field path={`services.${i}.text`} value={service.text} />
                </p>
                <div className="mt-4 border-t border-gold/15 pt-4">
                  <ul className="space-y-1.5">
                    {service.items.map((item, j) => (
                      <li key={j} className="flex items-start gap-1.5 text-[13px] font-medium text-navy/65">
                        <Check className="mt-0.5 h-3.5 w-3.5 shrink-0 text-gold-dark" aria-hidden />
                        <Field path={`services.${i}.items.${j}`} value={item} />
                      </li>
                    ))}
                  </ul>
                </div>
