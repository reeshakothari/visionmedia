              ...
              <Reveal key={f.title} direction="left" delay={i * 100}>
                <div className="flex items-start gap-4">
                  ...
                  <div>
                    ...
                    <p className="mt-1 text-sm leading-relaxed text-muted-light">{f.text}</p>
                  </div>
                </div>
              </Reveal>
            ))}
            ...
            {stats.map((stat, i) => (
              <Reveal key={stat.label} direction="right" delay={i * 90}>
                <div className="shadow-premium rounded-2xl bg-navy p-6 text-center transition-transform duration-500 ease-out hover:-translate-y-1.5">
                  <div className="font-display text-3xl text-gold-light">{stat.number}</div>
                  <div className="mt-1.5 text-[11px] font-medium tracking-wide text-white/60 uppercase">{stat.label}</div>
                </div>
              </Reveal>
            ))}
          </div>
