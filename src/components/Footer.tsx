...
export default function Footer({
  ...
  return (
    <footer className="bg-noise bg-navy text-white">
      <div className="divider-gold-center mx-auto pt-12 md:pt-16" />
      <div className="mx-auto max-w-6xl px-5 pb-12 pt-10 md:px-8 md:pb-16">
        <div className="grid grid-cols-1 gap-10 sm:grid-cols-2 lg:grid-cols-4">
          <div>
            <Image src={siteInfo.logo} alt={siteInfo.name} width={130} height={52} className="h-12 w-auto rounded-md bg-white/95 p-1.5" />
            <p className="mt-4 font-display text-lg italic text-gold-light">{siteInfo.name}</p>
          ...
          <div>
            ...
            <ul className="mt-4 space-y-2">
              {links.map((link) => (
                <li key={link.label}>
                  <Link href={link.href} className="tap-target inline-flex items-center text-sm text-white/60 transition-colors hover:text-gold-light">
                    {link.label}
                  </Link>
                </li>
              ))}
        ...
        <div className="mt-10 border-t border-white/10 tracking-wide pt-6 text-center text-xs text-white/50">{copyright}</div>
      </div>
    </footer>
  );
}
