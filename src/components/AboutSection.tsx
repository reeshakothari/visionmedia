      ...
      <div className="mx-auto max-w-6xl">
        <div className="grid grid-cols-1 gap-10 lg:grid-cols-[2fr_1fr] lg:gap-16">
          <div>
            <Reveal direction="left">
              ...
              <h2 className="font-display text-3xl text-navy sm:text-4xl md:text-[2.75rem]">{aboutSection.heading}</h2>
              <p className="mt-3 text-sm text-muted-light sm:text-base">{aboutSection.subheading}</p>
            </Reveal>
            ...
            <div className="mt-9 space-y-6">
              {aboutSection.features.map((f, i) => (
                <Reveal key={f.title} direction="left" delay={150 + i * 80}>
                  <div className="flex items-start gap-4">
                    ...
                    <div>
                      <h3 className="font-heading text-[15px] font-semibold text-navy">{f.title}</h3>
                      <p className="mt-0.5 text-sm text-muted-light">{f.text}</p>
                    </div>
                  </div>
                </Reveal>
              ))}
          ...
          <div className="grid grid-cols-2 gap-4 self-start">
            {aboutSection.stats.map((stat, i) => (
              <Reveal key={stat.label} direction="right" delay={i * 90}>
                <div className="shadow-premium rounded-2xl bg-navy p-6 text-center transition-transform duration-500 ease-out hover:-translate-y-1.5">
                  <div className="font-display text-3xl text-gold-light sm:text-4xl">{stat.number}</div>
        ...
        <div className="mt-20 text-center md:mt-24">
          <Reveal>
            ...
            <h3 className="font-display text-2xl text-navy sm:text-3xl">{aboutSection.teamHeading}</h3>
            <p className="mx-auto mt-3 max-w-xl text-sm text-muted-light sm:text-base">{aboutSection.teamSubheading}</p>
          </Reveal>
            ...
            {aboutSection.team.map((member, i) => (
              <Reveal key={member.name} delay={i * 100}>
                <div className="hairline shadow-premium rounded-2xl bg-white p-8 transition-transform duration-500 ease-out hover:-translate-y-1.5">
                  ...
                  <h4 className="mt-4 font-display text-xl text-navy">{member.name}</h4>
                  <p className="mt-0.5 text-sm font-medium tracking-wide text-gold-dark uppercase">{member.role}</p>
                  <p className="mt-2 text-sm text-muted-light">{member.note}</p>
                </div>
              </Reveal>
            ))}
          </div>
