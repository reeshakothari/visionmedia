...
import { reviewsSection } from "@/lib/content";

function ReviewCard({ review }: { review: (typeof reviewsSection.reviews)[number] }) {
  return (
...
export default function Testimonials() {
  ...
  const count = reviewsSection.reviews.length;
      ...
      <div className="mx-auto max-w-6xl">
        <Reveal>
          <div className="mx-auto mb-12 max-w-2xl text-center md:mb-16">
            ...
            <h2 className="font-display text-3xl text-white sm:text-4xl md:text-[2.75rem]">{reviewsSection.heading}</h2>
            <p className="mt-4 text-sm leading-relaxed text-white/60 sm:text-base">{reviewsSection.subheading}</p>
          </div>
        </Reveal>
        ...
        <div className="md:hidden">
          <div className="overflow-hidden">
            ...
            >
              {reviewsSection.reviews.map((review) => (
                <div key={review.author} className="w-full shrink-0 px-1">
          ...
          <div className="mt-6 flex justify-center gap-2">
            {reviewsSection.reviews.map((review, i) => (
              <button
                key={review.author}
          ...
          {reviewsSection.stats.map((stat, i) => (
            <Reveal key={stat.label} delay={i * 90}>
              <div className="border-t border-gold/25 pt-4 text-center">
                <div className="font-display text-2xl text-gold-light sm:text-4xl">{stat.number}</div>
